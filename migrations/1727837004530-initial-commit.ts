...
export class InitialCommit1727837004530 implements MigrationInterface {
    name = 'InitialCommit1727837004530'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`clients\` (\`id\` varchar(36) NOT NULL, \`first_name\` varchar(100) NOT NULL, \`last_name\` varchar(100) NOT NULL, \`client_id_type\` enum ('dni', 'pass', 'cuil', 'cuit', 'other') NULL, \`client_id\` varchar(50) NULL, \`e_mail\` varchar(255) NULL, \`bookly_id\` varchar(50) NULL, \`external_id\` varchar(50) NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, INDEX \`idx_client_first_name\` (\`first_name\`), INDEX \`idx_client_last_name\` (\`last_name\`), UNIQUE INDEX \`idx_client_type_and_id\` (\`client_id_type\`, \`client_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
