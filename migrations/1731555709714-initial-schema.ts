...
export class InitialSchema1731555709714 implements MigrationInterface {
    name = 'InitialSchema1731555709714'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`clients\` (\`id\` varchar(36) NOT NULL, \`first_name\` varchar(100) NOT NULL, \`last_name\` varchar(100) NOT NULL, \`person_id_type\` enum ('dni', 'pass', 'cuil', 'cuit', 'other') NULL, \`person_id\` varchar(50) NULL, \`e_mail\` varchar(255) NULL, \`phone_number\` varchar(255) NULL, \`bookly_id\` varchar(50) NULL, \`external_id\` varchar(50) NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, INDEX \`idx_client_first_name\` (\`first_name\`), INDEX \`idx_client_last_name\` (\`last_name\`), UNIQUE INDEX \`idx_person_type_and_id\` (\`person_id_type\`, \`person_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
